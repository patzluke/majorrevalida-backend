...
public class StudentApplicantCapabilitiesRepository {
	...
	public StudentApplicant insertStudentApplicant(StudentApplicant studentApplicant) {
		StudentApplicant applicant = dslContext.insertInto(STUDENT_APPLICANT)
				...
				.set(STUDENT_APPLICANT.DATE_APPLIED, studentApplicant.getDateApplied()).returning().fetchOne()
	...
	public List<Course> selectAllCourses() {
		return dslContext
				.selectDistinct(COURSE.COURSE_ID.as("courseId"), COURSE.COURSE_CODE.as("courseCode"),
						COURSE.PROGRAM_CODE.as("programCode"), COURSE.DEPT_CODE.as("deptCode"),
						COURSE.COURSE_TITLE.as("courseTitle"))
				.from(MAJOR)

				.innerJoin(COURSE).on(MAJOR.COURSE_CODE.eq(COURSE.COURSE_CODE)).fetchInto(Course.class);
	}

	public List<Major> selectCourseMajors(Integer courseCode) {
		return dslContext.selectFrom(MAJOR).where(MAJOR.COURSE_CODE.eq(courseCode)).fetchInto(Major.class);
	}

	// -------------------------- GETTING THE AVAILABLE OF SCHOOL YEAR THAT CAN BE
	// ENROLL
	public AcademicYear getAvailableAcademicYear() {
		...
		return availableSchoolYear == null ? new AcademicYear(1, 1, LocalDate.now(), LocalDate.now(), 1, "Close")
				: availableSchoolYear;
